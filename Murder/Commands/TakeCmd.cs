...
namespace MurderSim.Commands
{
	public class TakeCmd : Cmd
	{
		...
		public override string Execute(Player chr, string[] input)
		{
			...
			{
				return "What are you trying to take?\r";
			}
			if (input.Length == 4)
			{
				if (input[3 - 1] != "from")
				{
					return $"Where do you take {input[2 - 1]} from?\r";
				}
			...
			if (item == null)
			{
				return $"Sorry, I can't find {itemId}\r"; // in case such item does not exist
			}
			chr.FindInventory().Put(item); // add item to inventory
			return $"You have taken {itemId} from {where}\r";
		}
